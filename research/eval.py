...
from constants import TASK_PRETTY, TASK_GROUPS, MODEL_GROUPS, MODEL_PRETTY, MODEL_EMOJI, TASK_METRICS_PRETTY, BASELINE_MODEL, MODEL_INFO, MODEL_IDS
...
def get_run_scores(task, model, results, ensemble):
    ...
    if task == "squadnl":
        ...
        with open(os.path.join("exports", "predictions", task, model, "scores.json")) as f:
            return [x / 100 for x in json.load(f)]

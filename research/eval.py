...
def get_runs_results(task, model, results, ensemble):
    ...
    if metric_name in ["accuracy", "f1"]:
        ...
        preds = [[label_set.index(x) if x in label_set else len(label_set) for seq in pred for x in seq] for pred in preds]  # type: ignore
...
def generate_table(tasks, models, ensemble, table, lazy):
    ...
    group_names_str = [f"\\multicolumn{{{len(g) * 2}}}{{{('c' if group == last_group else 'c||')}}}{{{group}}}" for group, g in task_groups.items()]
    ...
    task_names_str = [f"\\multicolumn{{2}}{{c{_get_sep(group, t)}}}{{\\textbf{{{TASK_PRETTY[t]}}}}}" for group, g in task_groups.items() for t in g]
    metric_names_str = [f"\\multicolumn{{1}}{{c}}{{RER}} & \\multicolumn{{1}}{{c{_get_sep(group, t)}}}{{{TASK_METRICS_PRETTY[t]}}}" for group, g in task_groups.items() for t in g]
    ...
    for group, group_models in MODEL_GROUPS.items():
        ...
        for model in group_models:
            ...
            scores_str = [_format_score(task, model, table[model][task]['mean'], model in best_task_models[task], False) for task in tasks]

            print(f"\\emoji{{{MODEL_EMOJI[model]}}} {MODEL_PRETTY[model]} & {rel_score_avg_str} & " + " & ".join(scores_str) + " \\\\")
